...
const Dashboard: React.FC = () => {
  ...
  const handleTagUpdate = (bucketName: string, newTags: Record<string, string>) => {
    setBuckets(prevBuckets =>
      prevBuckets.map(bucket =>
        bucket.name === bucketName
          ? { ...bucket, labels: newTags }
          : bucket
      )
    );
  };

  const totalSize = buckets.reduce((sum, bucket) => sum + bucket.size_bytes, 0);
  const oldestBucket = buckets.length > 0
    ? buckets.reduce((oldest, current) => 
        ...
        <Grid item xs={12} md={4}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h4">
                {oldestBucket ? formatDate(oldestBucket.created_at) : 'N/A'}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
                  ...
                  <TableRow key={bucket.name}>
                    ...
                    <TableCell>
                      <TagManager
                        ...
                        tags={bucket.labels}
                        onUpdate={(newTags) => handleTagUpdate(bucket.name, newTags)}
                      />
                    </TableCell>
                    <TableCell>
